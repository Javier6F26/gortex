# pg-schema-safety Specification

## Purpose
Schema version detection, migration locking and idempotency, and the read-only open mode that never executes DDL, for the PostgreSQL graph store. Established by `harden-pg-store`.
