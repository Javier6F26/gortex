...
func diffResponseFields(id string, p, cons Contract, provShape, consShape *Shape, provType, consType string) []ContractIssue {
	...
	for name, cf := range consFields {
		...
		if !typesCompatible(pf.Type, cf.Type) {
			issues = append(issues, ContractIssue{
...
// typesCompatible is an intentionally-loose comparison: two type
// expressions are compatible if their canonical forms match. We strip
// whitespace, optional markers (`?`, `| null`, pointer `*`), and
// generic qualifiers so the Go-side `*User` lines up with the TS-side
// `User | null` and the Python-side `User | None`. This is only the
// shallow view — Stage 4 can do structural comparison when both sides
// of the wire resolve to Shapes.
...
func canonicalType(t string) string {
	...
	// Drop package qualifier: "pkg.User" → "User".
