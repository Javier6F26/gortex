...
// `GortexNode` so a client can `MATCH (n:GortexNode)` to delete the import in
// one statement before re-loading.
//
// Output structure:
//
//	// Header comment with stats.
//	CREATE INDEX … (Neo4j 5.x compatible; ignored on Memgraph)
//	CREATE (:Function:GortexNode {id: …, …});
//	…
//	MATCH (a:GortexNode {id: …}), (b:GortexNode {id: …}) CREATE (a)-[:CALLS {…}]->(b);
//	…
//
// The MATCH+CREATE per edge is deliberately simple. For graphs > ~50K edges
// this becomes slow to load over Bolt; a CSV-bundle exporter is a future
// enhancement.
func WriteCypher(w io.Writer, g *graph.Graph, opts Options) (Stats, error) {
	...
	// Header.
	fmt.Fprintln(cw, "// Auto-generated by gortex export")
	fmt.Fprintf(cw, "// Nodes: %d, Edges: %d\n", len(nodes), len(edges))
	fmt.Fprintln(cw, "// Load with: cypher-shell -u neo4j -p <pw> -f this_file.cypher")
	fmt.Fprintln(cw, "//")
	fmt.Fprintln(cw, "// To wipe a previous export before re-loading:")
	fmt.Fprintln(cw, "//   MATCH (n:GortexNode) DETACH DELETE n;")
	fmt.Fprintln(cw)

	// An index on :GortexNode(id) speeds up the per-edge MATCH lookups by
	// orders of magnitude. Neo4j 5.x and Memgraph both accept this syntax;
	// older Neo4j 4.x will reject it but the rest of the script still works.
	fmt.Fprintln(cw, "CREATE INDEX gortex_node_id IF NOT EXISTS FOR (n:GortexNode) ON (n.id);")
	fmt.Fprintln(cw)

	// Nodes.
	if len(nodes) > 0 {
		fmt.Fprintln(cw, "// === Nodes ===")
	}
	for _, n := range nodes {
